...
export class ReallyKickTable {
	...
	static createPage(that){
		...
		that.preparePage("everywhere. Soon the flickering bud of a flame catches on the oil, and a ", clickable:false});
		that.preparePage("fire ", clickable:true, nextPage:LookAtFire});
		that.preparePage("begins to engulf the table leg you were just kicking. As it grows, the ", clickable:false});
		that.preparePage("room ", clickable:true, nextPage:LookAroundRoom});
		that.preparePage("becomes well lit (lit with light, not fire, for now).", clickable:false});
