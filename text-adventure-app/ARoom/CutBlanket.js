...
export class CutBlanket {
	...
	static createPage(that){
		that.preparePage("You take the blanket and ", clickable:false});
		...
		that.preparePage(" caught on ", clickable:false});
		that.preparePage("fire!", clickable:true, nextPage:LookAtFire});
		that.preparePage(" How could this possibly help? I mean, I guess the cut on your foot stopped bleeding so much. ", clickable:false});
		var healing = 1;
		if(ARoomData.ThrewRope){
			healing = 2;
			that.preparePage("And the burns on your hands don't hurt as bad. ", clickable:false});
		}
		ARoomData.Health = ARoomData.Health + healing;
		that.preparePage("But man are you gonna have a nasty infection later. Fine, you gain "+ healing + "health temporarily, putting you at "+ARoomData.Health+".", clickable:false});
