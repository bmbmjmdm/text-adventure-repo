...
export class LookAroundRoom {
	...
	static createPage(that){
		...
		if(ARoomData.ThrewRope && ARoomData.ThrewGlass && (ARoomData.ThrewBlanket || ARoomData.BurntBlanket || ARoomData.WrappedHandsFeet)){
			...
			that.state.toShowText.push({text:" you wonder if it was worth taking the time to do so many things. You're already feeling dizzy, and you still need to ", clickable:false});
			that.state.toShowText.push({text:"climb the rope to freedom!", clickable:true, nextPage:ClimbRope});
			...
			that.state.toShowText.push({text:" just above it? The ", clickable:false});
			...
			if(ARoomData.WrappedHandsFeet){
				that.state.toShowText.push({text:"but with your hands wrapped, you'll probably be fine.", clickable:false});
			}
			else{
			...
			if(!ARoomData.ThrewGlass){
				...
				that.state.toShowText.push({text:" you used to free yourself is also nearby", clickable:false});
				if(!(ARoomData.ThrewBlanket || ARoomData.BurntBlanket || ARoomData.WrappedHandsFeet)){
				that.state.toShowText.push({text:", as well as the ", clickable:false});
				...
				if(!(ARoomData.ThrewBlanket || ARoomData.BurntBlanket || ARoomData.WrappedHandsFeet)){
					that.state.toShowText.push({text:"The cozy-looking, probably not bed-bug-ridden ", clickable:false});
					that.state.toShowText.push({text:"blanket ", clickable:true, nextPage:LookAtBlanket});
					that.state.toShowText.push({text:"is also here.", clickable:false});
				}
			}
			...
			that.state.toShowText.push({text:"window ", clickable:true, nextPage:LookAtWindow});
			that.state.toShowText.push({text:"above the door. The ", clickable:false});
			that.state.toShowText.push({text:"blanket", clickable:true, nextPage:LookAtBlanket});
			that.state.toShowText.push({text:" is also visible, stains and all.", clickable:false});
			...
			that.state.toShowText.push({text:"to your left. The floor is also wood, worn from many years and dull to the touch. There's a loose line of ", clickable:false});
			that.state.toShowText.push({text:"rope ", clickable:true, nextPage:LookAtRope});
			...
			that.state.toShowText.push({text:" door ", clickable:true, nextPage:LookAtDoor});
			that.state.toShowText.push({text:"on the wall. There's something above the door, but it's too dark to make out. Lastly, there's some rags that pass for a ", clickable:false});
			that.state.toShowText.push({text:"blanket", clickable:true, nextPage:LookAtBlanket});
			that.state.toShowText.push({text:" in a dark, dank corner.", clickable:false});
		}
